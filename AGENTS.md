...
- Shared metadata (`okf/_posts/index.md`, `okf/index.md`, `okf/log.md`) is maintained on `master`, separately from content review. Do not include these files in content PR diffs. The user has authorized direct, validated metadata updates; this does not authorize merging posts, images, or concepts.
- Before validating posts or concepts, generate indexes locally with `okf_index.py --write`, then run `okf_lint.py --allow-draft --check-index`. Restore only the generated indexes to the branch's committed versions before staging content. CI materializes indexes the same way without committing them to the PR.
- Review creation is itself recordable before approval: `.github/workflows/okf-metadata.yml` runs `okf_metadata.py --write` on trusted `master` for PR updates and merged changes, validates and normally pushes only the three metadata files. Pending reviews link to their PRs in a clearly labeled section, not to absent local posts. The log records reviewed topics and open/merged/closed status while preserving existing manual entries; accepted first-parent content changes are also recorded from the documented cutover. Re-runs are idempotent. Never execute PR scripts or force-push. If maintenance fails, report the run rather than asking the user to resolve generated text manually.
