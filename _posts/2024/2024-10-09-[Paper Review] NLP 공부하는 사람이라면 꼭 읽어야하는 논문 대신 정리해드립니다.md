    ...
    3. **리워드 모델의 평가**: **Reward Model**은 모델이 생성한 응답에 대해 **안전성(Safety)**과 **도움성(Helpfulness)**을 평가합니다. (만약 응답이 안전하지 않거나, 유해한 정보를 포함할 경우 **안전성 기준**을 적용해 **안전한 응답**이 선택될 수 있도록 보상을 조정합니다.)
       ...
       - **Reward Function**: 리워드 모델은 보상 함수를 사용해 응답의 적절성을 판단합니다.
         ...
         * 리워드 점수는 아래와 같이 계산됩니다.
           ...
           R(g∣p)=R^c(g∣p)−βDKL(πθ(g∣p)∥π0(g∣p))R(g \mid p) = \hat{R}\_c(g \mid p) - \beta D\_{KL}(\pi\_{\theta}(g \mid p) \parallel \pi\_0(g \mid p))R(g∣p)=R^c​(g∣p)−βDKL​(πθ​(g∣p)∥π0​(g∣p))

           여기서 R^c(g∣p)\hat{R}\_c(g \mid p)R^c​(g∣p)는 리워드 점수, βDKL\beta D\_{KL}βDKL​는 보상 조정에 따른 정책의 안정성을 의미합니다.
