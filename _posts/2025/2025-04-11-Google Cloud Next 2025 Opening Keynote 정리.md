...
2025년 4월, 라스베이거스에서 열린 **Google Cloud Next 2025**는 단순한 기술 컨퍼런스를 넘어, AI 중심의 클라우드 혁신이 실질적으로 비즈니스에 어떤 영향을 주고 있는지를 보여주는 자리였습니다. 이번 키노트는 “Why not?”이라는 도전적인 질문으로 시작하며, 기술이 단순한 도구를 넘어서 **인류의 상상력을 실현하는 수단**이 되는 미래를 제시했습니다.
...
> 참고 링크:
>
> * [Google Cloud Next 2025 Keynote Recap](https://cloud.google.com/next)
...
**요약:**  
Google Cloud Next 2025 키노트는 **Veo 2** 기반으로 제작된 카운트다운 영상으로 시작했습니다. 이는 단순한 시간 알림의 기능을 넘어, Google의 최신 **생성형 비디오 AI 기술의 성능을 실시간으로 체험**하는 경험으로 연출되었습니다.
...
> “Veo는 단순한 딥러닝 기반 동영상 생성기가 아니라, **크리에이티브 전문가와 브랜드 마케터의 파트너**가 될 수 있다.”
...
**요약:**  
이번 Google Cloud Next 2025 키노트의 도입부는 **“Why not?”**이라는 짧지만 강력한 메시지로 시작되는 **영화 스타일의 오프닝 영상**이었습니다. 기술을 향한 인간의 꿈과 도전을 상징하는 이 질문은, 앞으로 소개될 Google Cloud AI 기술들이 기존의 한계를 어떻게 허물고 있는지를 시청각적으로 표현했습니다.
...
**요약:**  
Thomas Kurian (Google Cloud CEO)는 Google Cloud의 지난 1년간의 성과를 요약하며 키노트의 본론을 열었습니다. 이 세션은 Google이 **기술적 진보뿐 아니라 글로벌 인프라 확장과 고객 기반 확대**에 집중해 왔다는 점을 강조합니다.

**주요 성과 하이라이트:**

* ✅ **기능 개선**: 2024년 한 해 동안 Google Cloud 및 Workspace에 **3,000개 이상의 제품 기능 업데이트**
* ✅ **글로벌 리전 확장**: 클라우드 인프라를 **42개 리전**으로 확대
...
* Google Cloud는 단순한 클라우드 인프라 제공자에서 **AI 중심의 생산성 플랫폼**으로 변모하고 있음
* 특히 Workspace(문서, 메일, 스프레드시트 등)에서 AI 기능이 실사용으로 안착되었음을 강조
* 제품 개선, 고객 확장, 인프라 구축, 생태계 확장을 통해 **클라우드 AI의 대중화 기반이 마련되었음**을 시사
...
* 2013년부터 자체 개발한 **Tensor Processing Unit**의 최신 버전
* 1세대 대비 **3,600배 성능 향상**
...
![](https://velog.velcdn.com/images/euisuk-chung/post/01ee6453-577a-4714-936e-b2428db9dcce/image.png)
...
![](https://velog.velcdn.com/images/euisuk-chung/post/a35d96f0-bef0-4477-b508-94895c85171d/image.png)
...
![](https://velog.velcdn.com/images/euisuk-chung/post/ce7fcaa2-79ad-46e1-bc9b-61914a40379a/image.png)
...
* **“Hypercomputer”는 단순히 빠른 서버의 집합이 아니라,**  
  인프라 하드웨어 + 스토리지 + 네트워크 + 스케일러블 런타임까지 통합한 **AI 전용 수퍼컴퓨터 플랫폼**
...
* Google은 자체 연구 역량(DeepMind + Research)과 인프라를 결합해 **기술적 파운더십을 유지**하고 있음
...
* Vertex AI의 **Model Garden**에서 200개 이상의 파운데이션 모델 제공
...
* **BigQuery, NetApp, SAP, Oracle, Salesforce, ServiceNow, Workday 등**과의 통합 강화
...
* **에이전트(Agent)**는 단일 질문-응답 구조를 넘어, 다음 기능을 수행하는 **지능형 작업 단위**입니다:
  + 추론(Reasoning)
  + 계획(Planning)
  + 기억(Memory)
  + 도구 사용(Tool Use)
...
* 하나의 모델/에이전트가 다양한 **데이터 소스와 도구**를 표준 방식으로 접근할 수 있도록 하는 **통합 인터페이스**
...
* AI가 실시간으로 **이슈를 요약, 분류, 응대 경로 지정**해줌
...
1. 라스베이거스 스카이라인 이미지 불러오기
2. Veo의 드론샷 프리셋으로 영상 생성
3. 등장인물, 무대, 관객 등 개별 씬 생성
4. 실수로 등장한 스태프를 inpainting으로 삭제
5. Lyria로 배경 음악 2개 생성 → 원하는 분위기로 선택
6. 전체 씬 + 음악 조합으로 최종 예고편 완성
...
Yasmeen은 Colab + BigQuery + Vertex AI를 이용해 **기업의 현금 흐름 악화 원인을 자연어 기반으로 분석하는 에이전트 기반 워크플로우**를 시연했습니다.

1. SAP, Salesforce, Google Ads, PDF 청구서 등의 데이터를 BigQuery에 연결
2. 데이터 엔지니어링 에이전트를 통해 여러 소스에서 **멀티모달 테이블 생성**
3. 날짜 포맷 등 데이터 품질 문제 → Gemini 기반 자동 정제
4. PDF 문서에서 구매자 정보와 지불 조건 추출 (멀티모달 분석)
5. 분류 모델로 구매자 세그먼트 분리
6. Gemini의 ‘Thinking Model’ 기반 원인 분석 →  
   ➤ 원인은 “36개월 지불 조건이 도입된 신규 프로모션”
7. **향후 3개월 현금 흐름 예측** → 신상품 및 고객군 별로 분석
...
* 각 프로세스를 분담하는 **에이전트들이 상호 협력**하는 구조
* 예:
