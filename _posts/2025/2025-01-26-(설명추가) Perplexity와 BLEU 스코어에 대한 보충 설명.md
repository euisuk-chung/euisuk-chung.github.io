...
3. **정보 이론적 근거**:
   ...
   * Perplexity는 정보 이론에서 사용되는 엔트로피 HHH와 밀접하게 관련되어 있습니다:

     H=−1N∑i=1Nlog⁡P(wi)H = -\frac{1}{N} \sum\_{i=1}^N \log P(w\_i)H=−N1​i=1∑N​logP(wi​)
   * Perplexity는 이를 지수 함수로 변환한 값:
...
1. **nnn-그램 유사도 계산**:
   ...
   * 생성된 번역문에서 참조 번역문에 일치하는 nnn-그램의 비율을 계산:

     Pn=생성된 번역문에서 참조 번역문과 일치하는 n-그램 개수생성된 번역문의 n-그램 개수P\_n = \frac{\text{생성된 번역문에서 참조 번역문과 일치하는 $n$-그램 개수}}{\text{생성된 번역문의 $n$-그램 개수}}Pn​=생성된 번역문의 n-그램 개수생성된 번역문에서 참조 번역문과 일치하는 n-그램 개수​
