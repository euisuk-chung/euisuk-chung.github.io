...
여기서 $\ell$은 작업 지시, $o_t=(\mathcal I_t,p_t)$는 여러 카메라 영상과 로봇의 내부 상태를 포함한 현재 관측, $h_t$는 최근 상호작용 이력입니다. $\Phi_{\mathcal P}$는 활성화된 플러그인으로 문맥 $c_t$를 구성합니다. 정책 $\pi$는 행동 집합 $\mathcal A$에서 $a_t$를 선택합니다. $g_E$는 로봇 형태 $E$에 맞는 해석기이며, 내부 목표 상태 $s_t$를 이용해 실행 명령 $u_t$를 만듭니다. 실행 후 새 관측이 들어오므로 결과를 보고 수정하는 폐루프입니다. [3.1절, 식 (1)~(3)](https://arxiv.org/html/2609.10522v1#S3.SS1)
...
$\mathbf{x}_t$는 3차원 위치, $Q_t$는 회전행렬, $s_t=(\mathbf{x}_t,Q_t)$는 목표 자세입니다. $d_a$는 이동 방향, $r_a$는 회전축과 방향을 표현합니다. 이동 행동에서는 $r_a=0$, 회전 행동에서는 $d_a=0$입니다. $\sigma_t$와 $\theta_t$는 이동 거리와 회전 각도이며, $R_E$는 의미적 방향을 해당 로봇의 좌표계로 변환합니다. $[\cdot]_{\times}$는 벡터를 반대칭행렬로 바꾸는 연산이고, 행렬 지수 $\exp$는 이를 회전으로 변환합니다. $\Pi_E$는 작업 공간과 단계별 이동·회전 제한을 적용합니다.

따라서 이동 간격을 바꿀 때는 $\sigma_t$를 조정하고, 로봇을 바꿀 때는 좌표 변환과 하위 제어를 담당하는 해석기를 교체합니다. 그리퍼 행동은 이 자세 갱신을 우회해 열기·닫기 명령으로 연결됩니다. Franka는 접촉 시 힘과 움직임의 관계를 조절하는 impedance control, AgileX는 목표 자세에서 관절값을 구하는 inverse kinematics와 관절 목표 스트리밍을 사용한다고 설명합니다. 작업 공간과 테이블 높이 제한에 어긋나는 행동은 실행 전에 차단하도록 구성합니다. 이 제한은 논문이 제시한 구현 장치이며 모든 물리 위험에 대한 보장을 뜻하지 않습니다. [3.2절, 식 (4)](https://arxiv.org/html/2609.10522v1#S3.SS2)
...
1. **A shared interface for humans, agents, and policy learning:** 행동 단위를 GUI 버튼·키 입력에 대응시킵니다. 사람은 키보드로, computer-use agent는 GUI로, 일반 VLM agent는 행동 단위를 직접 예측해 조작합니다. 실행 전 관측과 행동 $(o_t,a_t)$를 저장하며, 대응하는 저수준 명령과 궤적도 보존할 수 있습니다. 같은 시연으로 의미 행동 정책과 연속 제어 정책을 학습시키는 비교가 가능해집니다.
...
4. **Baselines:** VLA인 $\pi_{0.5}$·GR00T, VLA 중심 에이전트 H-VLA·G-VLA, 프로그램/API 기반 CaP-X·RATS를 비교합니다. 학습 가능한 VLA와 FT는 같은 시연에서 변환한 연속 궤적 또는 의미 행동으로 학습합니다.
...
HTML에서 표 2가 누락되어 동일 버전 PDF 11쪽의 표를 확인했습니다. 10개 작업 평균은 ZS 89%, FT 86%, $\pi_{0.5}$ 39%, GR00T 35%, H-VLA 50%, G-VLA 13%, CaP-X 44%, RATS 57%입니다. 다만 banana→bowl에서는 ZS가 6/10으로 RATS의 8/10보다 낮습니다. 평균 우세가 모든 개별 작업의 우세를 의미하지는 않습니다.

sim-to-real에서 FT는 13/20, $\pi_{0.5}$와 GR00T는 각각 0/20입니다. 나머지 방법은 해당 행에 결과가 없으므로 0점으로 취급하지 않습니다. 환경 평균 역시 방법별로 포함된 조건이 달라, ZS 100%와 FT 88%를 동일한 다섯 조건의 평균처럼 비교하면 안 됩니다. ZS는 앞의 네 조건에서 각각 20/20이고 sim-to-real은 미보고입니다. 로봇별 평가에서 ZS는 Franka 48/50·AgileX 45/50, FT는 45/50·42/50입니다. [5.2절](https://arxiv.org/html/2609.10522v1#S5.SS2), [PDF 표 2, 11쪽](https://arxiv.org/pdf/2609.10522v1#page=11)
...
1. **Fine-grained control:** 블록 쌓기·peg insertion에서 보폭을 2cm에서 1cm로 줄이면 ZS는 60→82%, FT는 40→65%로 개선됩니다. 같은 시연으로 학습한 $\pi_{0.5}$는 18%, 추가 정밀 시연 학습 후 62%입니다. 의미 행동을 유지한 채 실행 해상도를 바꾼 실험입니다.
2. **Action composition:** 직교 방향 두 단위를 대각선 변위로 결합해 단계 수를 줄이며, 저자는 뚜렷한 성공률 저하 없이 실행할 수 있다고 보고합니다.
3. **Rotation extrapolation:** 15도씩 회전하는 단위를 사용합니다. 0도·45도 시연으로 학습한 FT는 보지 못한 90도 당근 방향에서 70%, $\pi_{0.5}$는 20%입니다. 작은 행동의 반복 조합으로 큰 회전 차이를 처리하는지 평가합니다.
...
**Reasoning-intensive tasks:** 컵 아래 숨은 블록 찾기와 문자 “SHOW” 배열에서는 Situated Planning을 쓰는 ZS가 85%, FT 단독 10%, $\pi_{0.5}$ 단독 0%입니다. 동일한 Gemini 하위 지시를 주면 FT는 70%, $\pi_{0.5}$는 5%가 됩니다. 작은 제어 모델 자체의 계획 능력과, 외부 계획을 수행하는 능력을 구분해 보여 줍니다.
...
이동 단위 여섯 개의 표기만 바꾸는 $2\times2$ 설계입니다. 의미 이름+동작 규칙, 의미 이름만, 임의 기호+규칙, 임의 기호만을 비교합니다. 규칙이 있으면 임의 기호도 기본 방식에 근접하지만, 기호만 주어 스스로 동작을 추론하게 하면 1/20 성공과 23.3%의 매핑 정확도를 보고합니다. 저자의 결론은 **명시적 물리 규칙이 grounding의 대부분을 제공하고 의미 이름은 유용한 사전지식으로 작동한다**는 것입니다. [5.4.4절](https://arxiv.org/html/2609.10522v1#S5.SS4.SSS4)

#### 5.5 Qualitative Analysis

새 물체, 배경·조명 변화, 혼잡한 장면, 공간 추론, 양팔 제어를 그림 11로 보여 줍니다. 문자 재배열과 양팔 서랍 조작은 같은 행동 인터페이스가 다른 작업 구조에도 쓰이는 예입니다. 정성 사례는 동작 양상을 설명하는 자료이며 추가적인 성공률 표본으로 계산하지 않습니다. $\pi_{0.5}$와의 정성 비교는 부록 7.3으로 이어집니다. [5.5절](https://arxiv.org/html/2609.10522v1#S5.SS5)
...
40 epoch, 학습률 $10^{-4}$, cosine schedule, warmup ratio 0.1, bf16, $256\times256$ 영상, 유효 batch size 32를 사용합니다. 24GB급 GPU에서 학습 가능하다고 설명하며, Qwen3.5-2B는 H200 한 장에서 2시간 미만이라고 보고합니다. **24GB급 GPU에서도 2시간 미만이라는 의미는 아닙니다.** 이 절은 학습 표본을 7.9K로 표현하고, 표 3은 실제 로봇 행동 단계를 7,774로 제시하므로 두 표기를 임의로 동일 숫자로 고쳐 쓰지 않습니다. [7.1절](https://arxiv.org/html/2609.10522v1#S7.SS1)
...
동일 시연을 학습한 Qwen3.5-2B 기반 FT와 $\pi_{0.5}$를 체스 말·테니스공·곰인형에서 비교합니다. 저자는 후자의 잡기·물체 상호작용 실패를 표시합니다. 같은 데이터 기반이라는 통제와, 선별된 사례라는 자료의 성격을 함께 확인해야 합니다. [7.3절](https://arxiv.org/html/2609.10522v1#S7.SS3)
...
표 2의 같은 10개 작업을 기준으로 보면, FT 86%와 $\pi_{0.5}$ 39%의 차이는 **47퍼센트포인트**, ZS 89%와 비교군 중 가장 높은 RATS 57%의 차이는 **32퍼센트포인트**입니다. 이는 표의 보고값에서 계산한 절대 차이입니다. FT와 VLA에 같은 시연을 사용한 설계는 데이터 수집량 차이의 영향을 줄이지만, 서로 다른 모델과 학습 표현을 모두 동일하게 만든 실험은 아닙니다. [PDF 표 2](https://arxiv.org/pdf/2609.10522v1#page=11)
...
**통계적 유의성 검정과 신뢰구간은 논문에 보고되지 않았습니다.** 기본 작업당 10회이므로 개별 작업의 성공 한 번 차이는 10퍼센트포인트입니다. 이 관찰은 표본의 해석을 위한 설명이며 별도의 유의성 검정 결과가 아닙니다. 한편 손목 영상·내부 상태·복구 제거 실험과 행동 표현의 $2\times2$ 비교는 어떤 구성요소가 결과에 기여하는지 살펴보는 근거가 됩니다.
