...
def protect_liquid_in_code_blocks(content):
    """Liquid 구문이 든 코드블록을 {% raw %}로 감싼다.

    Jekyll은 마크다운 본문도 Liquid로 먼저 렌더한다. 프롬프트 예시처럼
    `{{PLACEHOLDER}}` 가 들어간 코드블록을 그대로 두면 Jekyll이 이를
    템플릿 변수로 해석해 조용히 지워버린다(빈 문자열로 치환).
    코드블록 단위로 raw 처리해 원문을 보존한다.
    """
    lines = content.split("\n")
    fences, start = [], None
    ...
    def already_wrapped(a, b):
        # raw 태그는 펜스 바깥에 붙으므로 앞뒤 줄을 본다.
        before = lines[a - 1].strip() if a > 0 else ""
        after = lines[b + 1].strip() if b + 1 < len(lines) else ""
        return before == "{% raw %}" and after == "{% endraw %}"

    targets = {
        a: b for a, b in fences
        if any(liquid.search(l) for l in lines[a + 1:b])
        and not already_wrapped(a, b)
    }
    if not targets:
        return content
